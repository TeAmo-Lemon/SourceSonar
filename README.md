...
- 新闻采集：支持 RSS / JSON / 网页热点源，Crawl4AI + Playwright 正文补抓并同步采集封面图与正文配图，管理后台可视化维护新闻源
- 热点列表：按热度/时间浏览新闻，支持时间范围、分类、地区、来源筛选
- 语义搜索：向量召回 + 文本匹配，支持自然语言查询
- AI 分析：使用硅基流动 Qwen3-VL 联合分析新闻文本与图片，生成情感、视觉依据、分类、关键词和实体，并对同一事件多来源报道去重聚合
...
- `MULTIMODAL_SENTIMENT_*`：硅基流动 Qwen3-VL 多模态情感模型、图片数量、细节等级、并发和超时
- `AUTO_*`：自动任务参数
- `TOPIC_*`：专题发现参数
- `CRAWLER_*`：爬虫并发/超时/重试
